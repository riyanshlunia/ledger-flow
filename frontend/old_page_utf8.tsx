...
// ΓöÇΓöÇΓöÇ View type ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
type View = "dashboard" | "reconciliation" | "forecasts" | "escalations" | "audit" | "scenarios";

// ΓöÇΓöÇΓöÇ Utility helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Confidence bar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            ...
            {stats.balance_as_of ? fmtDate(stats.balance_as_of) : "ΓÇô"}
          </p>
        </div>
...
// ΓöÇΓöÇΓöÇ Reconciliation View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    <>
      <div className="page-header">
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button id="btn-run-reconciliation" className="btn btn-primary btn-sm" onClick={runAgent} disabled={running}>
            {running ? <><div className="spinner" style={{ width: 12, height: 12 }} /> RunningΓÇª</> : " Run Agent"}
          </button>
        </div>
      </div>
      ...
      <div className="page-body">
        <div style={{ display: "flex", gap: 20 }}>
          <div style={{ flex: 1 }}>
            {loading ? (
              <div className="loading-state"><div className="spinner" />LoadingΓÇª</div>
              ...
              <>
                    ...
                    <tbody>
                      {data.items.map((m) => (
                        <tr key={m.match_id} style={{ cursor: "pointer" }} onClick={() => openExplain(m.match_id)}>
                          <td className="td-mono">{m.bank_tx_id.slice(0, 8)}ΓÇª</td>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 12, color: "var(--text-muted)" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button className="btn btn-ghost btn-xs" disabled={page <= 1} onClick={() => setPage(p => p - 1)}>ΓåÉ Prev</button>
                    <span style={{ padding: "3px 8px" }}>Page {page}</span>
                    <button className="btn btn-ghost btn-xs" disabled={data.items.length < 50} onClick={() => setPage(p => p + 1)}>Next ΓåÆ</button>
                  </div>
                </div>
              </>
            )}
            ...
            <div style={{ width: 360, flexShrink: 0 }}>
              <div className="card" style={{ position: "sticky", top: 24 }}>
                <div className="card-header">
                  ...
                  <button className="btn btn-ghost btn-xs" onClick={() => { setSelected(null); setExplain(null); }}>Γ£ò</button>
                </div>
                {explainLoading ? (
                  <div className="loading-state" style={{ padding: 24 }}><div className="spinner" />LoadingΓÇª</div>
                ) : explain ? (
...
// ΓöÇΓöÇΓöÇ Forecast View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function ForecastView() {
  ...
  if (loading) return <div className="loading-state"><div className="spinner" />LoadingΓÇª</div>;
    ...
    <>
      <div className="page-header">
        ...
        <button id="btn-run-forecast" className="btn btn-primary btn-sm" onClick={runForecast} disabled={running}>
          {running ? <><div className="spinner" style={{ width: 12, height: 12 }} /> RunningΓÇª</> : " Run Forecast"}
        </button>
      </div>
        ...
        {buckets.length === 0 ? (
          ...
          <>
            ...
            <div className="card mb-24">
              <div className="card-header">
                <span className="card-title">Weekly Cash Flow ΓÇö Inflows vs Outflows</span>
                <div style={{ display: "flex", gap: 16, fontSize: 11 }}>
                  <span><span style={{ color: "#10b981" }}>Γûá</span> Inflows</span>
                  <span><span style={{ color: "#ef4444" }}>Γûá</span> Outflows</span>
                  <span><span style={{ color: "#3b82f6" }}>Γûá</span> Net</span>
                </div>
              </div>
                      ...
                      {buckets.map((b) => (
                        <tr key={b.forecast_id}>
                          <td style={{ fontWeight: 600 }}>
                            {fmtDate(b.period_start)} ΓÇô {fmtDate(b.period_end)}
                          </td>
                          ...
                          <td style={{ fontSize: 11, color: "var(--text-muted)" }}>
                            {fmtCurrency(b.p10_closing)} ΓÇô {fmtCurrency(b.p90_closing)}
                          </td>
                          <td>
                            <button
                ...
                <div style={{ width: 320, flexShrink: 0 }}>
                  <div className="card" style={{ position: "sticky", top: 24 }}>
                    <div className="card-header">
                      ...
                      <button className="btn btn-ghost btn-xs" onClick={() => { setSelected(null); setDrivers(null); }}>Γ£ò</button>
                    </div>
                    {!drivers ? (
                      <div className="loading-state" style={{ padding: 24 }}><div className="spinner" />LoadingΓÇª</div>
                    ) : (
...
// ΓöÇΓöÇΓöÇ Escalations View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                <div key={e.escalation_id} className={`escalation-card severity-${e.severity}`}>
                  <div className="escalation-header">
                    ...
                    <div style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-muted)" }}>
                      {e.escalation_id.slice(0, 12)}ΓÇª
                    </div>
                  </div>
...
// ΓöÇΓöÇΓöÇ Audit View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function AuditView() {
            ...
            <>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16, fontSize: 12, color: "var(--text-muted)" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="btn btn-ghost btn-xs" disabled={page <= 1} onClick={() => setPage(p => p - 1)}>ΓåÉ Prev</button>
                  <span style={{ padding: "3px 8px" }}>Page {page}</span>
                  <button className="btn btn-ghost btn-xs" disabled={data.entries.length < 50} onClick={() => setPage(p => p + 1)}>Next ΓåÆ</button>
                </div>
              </div>
            </>
          )}
...
// ΓöÇΓöÇΓöÇ Scenarios View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">What-if analysis ΓÇö stress test cash flow against policy rules</p>
        </div>
      </div>
              ...
              <div style={{ width: 400, flexShrink: 0 }}>
                <div className="card" style={{ position: "sticky", top: 24 }}>
                  <div className="card-header">
                    ...
                    <button className="btn btn-ghost btn-xs" onClick={() => { setSelected(null); setDelta(null); }}>Γ£ò</button>
                  </div>
                  {deltaLoading ? (
                    <div className="loading-state" style={{ padding: 24 }}><div className="spinner" />Running scenarioΓÇª</div>
                  ) : delta ? (
                    <div>
...
// ΓöÇΓöÇΓöÇ App Shell ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
